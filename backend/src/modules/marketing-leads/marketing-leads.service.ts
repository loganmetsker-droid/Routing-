...
export class MarketingLeadsService {
  ...
  async create(dto: CreateMarketingLeadDto) {
    ...
    this.audit?.record({
      ...
      entityId: lead.id,
      action: 'pilot.funnel.lead-submitted',
      source: 'integration',
      newValue: {
  ...
  private async notifyOperator(lead: MarketingLead) {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `Postmark returned ${response.status}${receipt?.Message ? `: ${receipt.Message}` : ''}`,
        );
      }
      lead.notificationStatus = 'sent';
      lead.notificationError = null;
      lead.notificationMessageId = receipt.MessageID;
    } catch (error) {
