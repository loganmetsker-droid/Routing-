...
function createController(summary = runtimeSummary()) {
  ...
  return new HealthController(
    ...
    { checkAvailability: vi.fn().mockResolvedValue(true) } as never,
    { checkR2Availability: vi.fn().mockResolvedValue(true) } as never,
  );
}
...
describe('HealthController readiness', () => {
  ...
  it('returns 503 when a configured hosted provider is unavailable', async () => {
    const controller = createController();
    (controller as any).workosService.checkAvailability.mockResolvedValue(false);
    const response = { status: vi.fn() };
    const result = await controller.readiness(response as never);

    expect(response.status).toHaveBeenCalledWith(503);
    expect(result.missingCritical).toContain('workos');
    expect(result.dependencies.workos.status).toBe('down');
  });
});
