...
npm run release:build-candidate
node scripts/release-candidate-check.mjs --directory /absolute/path/to/candidate
```

The snapshot writes a tracked diff, a tarball of non-secret untracked files, hashes, and a manifest outside the repository. Treat the printed candidate directory as the release source and rebuild it after any source or runbook change.
...
Before hosted smoke, verify the candidate uses `GEOCODING_PROVIDER=mapbox`, `ROUTING_MATRIX_PROVIDER=osrm`, `ROUTING_MATRIX_ALLOW_FALLBACK=false`, a non-public matrix base URL, an error-monitor destination, and a valid access-code encryption key. If legacy plaintext access codes exist, run and verify the guarded access-code backfill before onboarding.
