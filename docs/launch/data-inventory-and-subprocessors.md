...
The restore drill must establish actual Postgres backup retention and restore behavior. The R2 recovery check must establish object version/lifecycle behavior. Render log retention, audit retention, customer export timing, customer deletion timing, and backup-expiry timing must then be copied into approved privacy, retention, support, and pilot agreement wording. Do not publish guessed periods.
