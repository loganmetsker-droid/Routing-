...
type JobFormData = {
  ...
  stackable: boolean;
  fragile: boolean;
  maxStackLevels: string;
  maxStackHeightIn: string;
  additionalPalletGroups: Array<{
    id: string;
    label: string;
    quantity: string;
    lengthIn: string;
...
const buildRoutingRequirements = (
  ...
  const primaryPalletGroup: PalletLoadUnit | null = [
    formData.palletCount,
    formData.palletLengthIn,
    formData.palletWidthIn,
    formData.palletHeightIn,
    formData.palletWeightLb,
  ].every((value) => optionalNumber(value))
    ? {
        id: 'primary-pallet-group',
        label: 'Primary pallets',
        quantity: Number(formData.palletCount),
        lengthIn: Number(formData.palletLengthIn),
        widthIn: Number(formData.palletWidthIn),
        heightIn: Number(formData.palletHeightIn),
        weightLb: Number(formData.palletWeightLb),
        stackable: formData.stackable,
        fragile: formData.fragile,
        maxStackLevels: optionalNumber(formData.maxStackLevels),
      }
    : null;
  const additionalPalletGroups = formData.additionalPalletGroups.flatMap(
    (group): PalletLoadUnit[] => {
      ...
      return [{
        id: group.id,
        label: group.label.trim() || 'Pallet group',
        quantity: values[0] as number,
        lengthIn: values[1] as number,
        widthIn: values[2] as number,
        heightIn: values[3] as number,
        weightLb: values[4] as number,
        stackable: group.stackable,
        fragile: group.fragile,
        maxStackLevels: optionalNumber(group.maxStackLevels),
      }];
    },
  );
  return {
    ...
    fragile: formData.fragile,
    maxStackLevels: optionalNumber(formData.maxStackLevels),
    maxStackHeightIn: optionalNumber(formData.maxStackHeightIn),
    palletGroups: [
      ...(primaryPalletGroup ? [primaryPalletGroup] : []),
      ...additionalPalletGroups,
    ],
  },
  requiredEquipment: parseEquipmentList(formData.requiredEquipment),
  requiredDriverId: formData.requiredDriverId || undefined,
  requiredDriverName: formData.requiredDriverName.trim() || undefined,
  vehicle: {
    requiredFeatures: parseEquipmentList(formData.requiredVehicleFeatures),
    allowedVehicleIds: formData.allowedVehicleIds,
    prohibitedVehicleIds: formData.prohibitedVehicleIds,
  },
  driver: {
    allowedDriverIds: formData.allowedDriverIds,
    prohibitedDriverIds: formData.prohibitedDriverIds,
    requiredCertifications: parseEquipmentList(formData.requiredCertifications),
  },
  sequence: {
    position: formData.stopPosition,
    strict: formData.stopPosition !== 'any',
  },
  site: {
    accessNotes: formData.siteAccessNotes.trim() || undefined,
    accessCode: formData.accessCode.trim() || undefined,
    accessCodeRequired: formData.accessCodeRequired,
    gateInstructions: formData.gateInstructions.trim() || undefined,
...
const extractRoutingRequirements = (
  ...
  const list = (...keys: unknown[]) =>
    String(keys.find((value) => value !== undefined && value !== null) || '')
      .split(/[;,]/)
      .map((item) => item.trim())
      .filter(Boolean);
  const rawSequence = String(
    record.stopPosition || record.stop_position || record.sequencePosition || 'any',
  ).trim().toLowerCase();
  const sequencePosition = ['first', 'last'].includes(rawSequence)
    ? rawSequence as 'first' | 'last'
    : 'any';
  ...
  return {
    load: {
      ...
      fragile: optionalBoolean(record.fragile),
      maxStackLevels: optionalNumber(record.maxStackLevels ?? record.max_stack_levels),
      maxStackHeightIn: optionalNumber(record.maxStackHeightIn ?? record.max_stack_height_in),
    ...
    requiredEquipment: list(record.requiredEquipment, record.required_equipment),
    requiredDriverId: String(record.requiredDriverId || record.required_driver_id || '').trim() || undefined,
    requiredDriverName: String(record.requiredDriverName || record.required_driver || '').trim() || undefined,
    vehicle: {
      requiredFeatures: list(record.requiredVehicleFeatures, record.required_vehicle_features),
      allowedVehicleIds: list(record.allowedVehicleIds, record.allowed_vehicle_ids),
      prohibitedVehicleIds: list(record.prohibitedVehicleIds, record.prohibited_vehicle_ids),
    },
    driver: {
      allowedDriverIds: list(record.allowedDriverIds, record.allowed_driver_ids),
      prohibitedDriverIds: list(record.prohibitedDriverIds, record.prohibited_driver_ids),
      requiredCertifications: list(record.requiredCertifications, record.required_certifications),
    },
    sequence: {
      position: sequencePosition,
      strict: sequencePosition !== 'any',
    },
    site: {
      accessNotes: String(record.siteAccessNotes || record.site_access || record.accessNotes || '').trim() || undefined,
      accessCode: String(record.accessCode || record.access_code || '').trim() || undefined,
      accessCodeRequired: optionalBoolean(record.accessCodeRequired ?? record.access_code_required),
      gateInstructions: String(record.gateInstructions || record.gate_instructions || '').trim() || undefined,
...
const hasSpecialHandling = (job: JobRecord) =>
  Boolean(
    job.routingRequirements?.requiredEquipment?.length ||
      job.routingRequirements?.requiredDriverId ||
...
export default function JobsPageEnhancedV2() {
  ...
  const focusedCustomer = focusedJob
    ...
    : null;
  const focusedVehicleFit = useMemo(() => {
    ...
    return vehicles
      ...
      .sort((left, right) => {
        if (left.fit.fits !== right.fit.fits) return left.fit.fits ? -1 : 1;
        return (
          (left.fit.utilization.palletPositionPercent || 999) -
          (right.fit.utilization.palletPositionPercent || 999)
        );
      });
  }, [drivers, focusedJob, vehicles]);
  const queueCounts = {
    ...
    try {
      await Promise.all(
        importCandidates.map(async (candidate) => {
          ...
          const requiredDriverId = candidate.routingRequirements?.requiredDriverId?.trim();
          const requiredDriverName = candidate.routingRequirements?.requiredDriverName?.trim();
          const matchedDriver = drivers.find((driver) => {
            if (requiredDriverId && driver.id === requiredDriverId) return true;
            if (!requiredDriverName) return false;
            const requested = requiredDriverName.toLowerCase();
            return formatPersonName(driver).toLowerCase() === requested ||
              String(driver.email || '').toLowerCase() === requested;
          });
          if ((requiredDriverId || requiredDriverName) && !matchedDriver) {
            throw new Error(
              `Required driver "${requiredDriverName || requiredDriverId}" was not found in this workspace.`,
            );
          }
          const routingRequirements = candidate.routingRequirements
            ? {
                ...candidate.routingRequirements,
                requiredDriverId: matchedDriver?.id || requiredDriverId,
                requiredDriverName: matchedDriver
                  ? formatPersonName(matchedDriver)
                  : requiredDriverName,
              }
            : candidate.routingRequirements;
      ...
      setImportError(
        error instanceof Error
          ? error.message
          : 'One or more jobs could not be imported. Check the file format and try again.',
      );
    } finally {
      setImporting(false);
    }
  ...
  return (
    ...
    >
          ...
          {focusedJob ? (
            <Stack spacing={1.25}>
              ...
              <Stack spacing={1.1} sx={{ px: 1.2, pb: 1.2 }}>
                ...
                <SurfacePanel variant="muted" padding={1}>
                  <Typography variant="subtitle2">Fleet fit estimate</Typography>
                  <Typography variant="caption" color="text.secondary">
                    Rough planning estimate from configured weight, volume, pallet floor positions, stack limits, features, and driver rules. Confirm axle balance and securement before loading.
                  </Typography>
                  <Stack spacing={0.8} sx={{ mt: 1 }} data-testid="job-fleet-fit-estimate">
                    {focusedVehicleFit.length ? focusedVehicleFit.slice(0, 6).map(({ vehicle, driver, fit }) => (
                      <Box
                        key={vehicle.id}
                        sx={{
                          display: 'grid',
                          gridTemplateColumns: { xs: '1fr', sm: 'minmax(150px, 0.8fr) minmax(220px, 1.2fr) auto' },
                          gap: 0.75,
                          alignItems: 'center',
                          p: 0.8,
                          border: '1px solid',
                          borderColor: fit.fits ? 'success.main' : 'divider',
                          borderRadius: 1,
                        }}
                      >
                        <Box>
                          <Typography variant="body2" sx={{ fontWeight: 800 }}>{formatVehicleName(vehicle)}</Typography>
                          <Typography variant="caption" color="text.secondary">
                            {driver ? formatPersonName(driver) : 'Driver not assigned'}
                          </Typography>
                        </Box>
                        <Typography variant="caption" color="text.secondary">
                          {fit.totals.palletCount} pallets / ~{fit.totals.floorPositionsNeeded} floor positions
                          {' • '}{Math.round(fit.totals.weightKg * 2.2046226218).toLocaleString()} lb
                          {fit.blockers[0] ? ` • ${fit.blockers[0].message}` : fit.warnings[0] ? ` • ${fit.warnings[0].message}` : ''}
                        </Typography>
                        <StatusPill label={fit.fits ? 'Fits' : 'Blocked'} tone={fit.fits ? 'success' : 'danger'} />
                      </Box>
                    )) : (
                      <Alert severity="info">Add vehicle cargo profiles to compare this load across the fleet.</Alert>
                    )}
                  </Stack>
                </SurfacePanel>
      ...
      <Dialog open={dialogOpen} onClose={() => updateUrl({ create: null })} fullWidth maxWidth="md">
        ...
        <DialogContent sx={{ display: 'grid', gap: 2, pt: 2 }}>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
            ...
            />
            <FormControlLabel
              control={<Checkbox checked={formData.fragile} onChange={(event) => setFormData((current) => ({ ...current, fragile: event.target.checked }))} />}
              label="Fragile / do not double-stack"
            />
            <TextField label="Max stack levels" type="number" value={formData.maxStackLevels} onChange={(event) => setFormData((current) => ({ ...current, maxStackLevels: event.target.value }))} fullWidth />
            <TextField label="Max stack height (in)" type="number" value={formData.maxStackHeightIn} onChange={(event) => setFormData((current) => ({ ...current, maxStackHeightIn: event.target.value }))} fullWidth />
          </Stack>
          <Stack spacing={1} data-testid="additional-pallet-groups">
            <Stack direction="row" justifyContent="space-between" alignItems="center">
              <Box>
                <Typography variant="subtitle2">Additional pallet sizes</Typography>
                <Typography variant="caption" color="text.secondary">Add a group when one stop contains mixed pallet dimensions or handling rules.</Typography>
              </Box>
              <Button
                variant="outlined"
                size="small"
                onClick={() => setFormData((current) => ({
                  ...current,
                  additionalPalletGroups: [
                    ...current.additionalPalletGroups,
                    {
                      id: `pallet-group-${Date.now()}`,
                      label: '',
                      quantity: '1',
                      lengthIn: '48',
                      widthIn: '40',
            ...
            {formData.additionalPalletGroups.map((group, index) => (
              <SurfacePanel key={group.id} variant="muted" padding={1}>
                <Grid container spacing={1} alignItems="center">
                  <Grid item xs={12} md={3}><TextField label="Group label" value={group.label} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item) }))} fullWidth /></Grid>
                  <Grid item xs={4} md={1.5}><TextField label="Count" type="number" value={group.quantity} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, quantity: event.target.value } : item) }))} fullWidth /></Grid>
                  <Grid item xs={4} md={1.5}><TextField label="L in" type="number" value={group.lengthIn} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, lengthIn: event.target.value } : item) }))} fullWidth /></Grid>
                  <Grid item xs={4} md={1.5}><TextField label="W in" type="number" value={group.widthIn} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, widthIn: event.target.value } : item) }))} fullWidth /></Grid>
                  <Grid item xs={4} md={1.5}><TextField label="H in" type="number" value={group.heightIn} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, heightIn: event.target.value } : item) }))} fullWidth /></Grid>
                  <Grid item xs={4} md={1.5}><TextField label="Each lb" type="number" value={group.weightLb} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, weightLb: event.target.value } : item) }))} fullWidth /></Grid>
                  <Grid item xs={4} md={1.5}><Button color="error" onClick={() => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.filter((_, itemIndex) => itemIndex !== index) }))}>Remove</Button></Grid>
                  <Grid item xs={12}>
                    <Stack direction="row" spacing={1.5} flexWrap="wrap" useFlexGap>
                      <FormControlLabel control={<Checkbox checked={group.stackable} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, stackable: event.target.checked } : item) }))} />} label="Stackable" />
                      <FormControlLabel control={<Checkbox checked={group.fragile} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, fragile: event.target.checked } : item) }))} />} label="Fragile" />
                      <TextField size="small" label="Max levels" type="number" value={group.maxStackLevels} onChange={(event) => setFormData((current) => ({ ...current, additionalPalletGroups: current.additionalPalletGroups.map((item, itemIndex) => itemIndex === index ? { ...item, maxStackLevels: event.target.value } : item) }))} />
                    </Stack>
                  </Grid>
                </Grid>
              </SurfacePanel>
          ...
          <Divider />
          <Typography variant="subtitle2">Vehicle and driver rules</Typography>
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
            ...
            <TextField label="Required vehicle features" value={formData.requiredVehicleFeatures} placeholder="side door, dock height" onChange={(event) => setFormData((current) => ({ ...current, requiredVehicleFeatures: event.target.value }))} fullWidth />
            <TextField
              select
              label="Required driver"
              value={formData.requiredDriverId}
              onChange={(event) => {
                const requiredDriverId = event.target.value;
                const driver = drivers.find((candidate) => candidate.id === requiredDriverId);
                setFormData((current) => ({
                  ...current,
                  requiredDriverId,
                  requiredDriverName: driver ? formatPersonName(driver) : '',
                }));
              }}
              fullWidth
            >
              ...
              {drivers.map((driver) => (
                <MenuItem key={driver.id} value={driver.id}>{formatPersonName(driver)}</MenuItem>
              ))}
            </TextField>
            <TextField label="Required driver certifications" value={formData.requiredCertifications} placeholder="hazmat, glass handling" onChange={(event) => setFormData((current) => ({ ...current, requiredCertifications: event.target.value }))} fullWidth />
          </Stack>
          <Grid container spacing={1.5}>
            <Grid item xs={12} md={6}>
              <TextField select label="Only these vehicles" value={formData.allowedVehicleIds} onChange={(event) => setFormData((current) => ({ ...current, allowedVehicleIds: typeof event.target.value === 'string' ? event.target.value.split(',') : event.target.value }))} SelectProps={{ multiple: true }} fullWidth>
                {vehicles.map((vehicle) => <MenuItem key={vehicle.id} value={vehicle.id}>{formatVehicleName(vehicle)}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField select label="Never use these vehicles" value={formData.prohibitedVehicleIds} onChange={(event) => setFormData((current) => ({ ...current, prohibitedVehicleIds: typeof event.target.value === 'string' ? event.target.value.split(',') : event.target.value }))} SelectProps={{ multiple: true }} fullWidth>
                {vehicles.map((vehicle) => <MenuItem key={vehicle.id} value={vehicle.id}>{formatVehicleName(vehicle)}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField select label="Only these drivers" value={formData.allowedDriverIds} onChange={(event) => setFormData((current) => ({ ...current, allowedDriverIds: typeof event.target.value === 'string' ? event.target.value.split(',') : event.target.value }))} SelectProps={{ multiple: true }} fullWidth>
                {drivers.map((driver) => <MenuItem key={driver.id} value={driver.id}>{formatPersonName(driver)}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField select label="Never assign these drivers" value={formData.prohibitedDriverIds} onChange={(event) => setFormData((current) => ({ ...current, prohibitedDriverIds: typeof event.target.value === 'string' ? event.target.value.split(',') : event.target.value }))} SelectProps={{ multiple: true }} fullWidth>
                {drivers.map((driver) => <MenuItem key={driver.id} value={driver.id}>{formatPersonName(driver)}</MenuItem>)}
              </TextField>
            </Grid>
          </Grid>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
            <TextField label="Gate / driveway access code" type="password" value={formData.accessCode} onChange={(event) => setFormData((current) => ({ ...current, accessCode: event.target.value }))} helperText="Visible only inside the authenticated route workflow." fullWidth />
            <TextField label="Gate instructions" value={formData.gateInstructions} placeholder="Keypad on the left after the cattle guard" onChange={(event) => setFormData((current) => ({ ...current, gateInstructions: event.target.value }))} fullWidth />
            <TextField select label="Stop position" value={formData.stopPosition} onChange={(event) => setFormData((current) => ({ ...current, stopPosition: event.target.value as JobFormData['stopPosition'] }))} fullWidth>
              <MenuItem value="any">Optimizer decides</MenuItem>
              <MenuItem value="first">Must be first</MenuItem>
              <MenuItem value="last">Must be last</MenuItem>
            </TextField>
          </Stack>
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
            <FormControlLabel
              control={<Checkbox checked={formData.accessCodeRequired} onChange={(event) => setFormData((current) => ({ ...current, accessCodeRequired: event.target.checked }))} />}
      ...
      <Dialog open={importDialogOpen} onClose={() => updateUrl({ import: null })} fullWidth maxWidth="md">
        ...
        <DialogContent sx={{ display: 'grid', gap: 2, pt: 2 }}>
          <Alert severity="info">
            Import JSON arrays or CSV files with customerName, deliveryAddress, time windows, serviceDuration, pallet dimensions/weight, fragile, stack limits, required equipment/features/certifications, vehicle or driver allow/block lists, accessCode, gateInstructions, stopPosition, temperature, hazmat, and handling columns.
          </Alert>
