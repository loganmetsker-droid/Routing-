...
function parseCompletionLocationEvidence(
  events: StopEventRecord[],
): CompletionLocationEvidence | null {
  const servicedEvents = events
    .filter((event) => String(event.eventType).toUpperCase() === 'SERVICED')
    .sort((left, right) =>
      String(right.happenedAt || '').localeCompare(String(left.happenedAt || '')),
    );
  for (const event of servicedEvents) {
    const value = event.payload?.completionLocationEvidence;
    ...
    return {
      status,
      thresholdMeters: Number(record.thresholdMeters || 250),
      varianceMeters:
        typeof record.varianceMeters === 'number' ? record.varianceMeters : null,
      telemetryAt:
        typeof record.telemetryAt === 'string' ? record.telemetryAt : null,
...
function CompletionLocationEvidencePanel({
  evidence,
  stopIndex,
}: {
  evidence: CompletionLocationEvidence;
  stopIndex: number;
}) {
  const isAttention = evidence.status === 'attention';
  const isVerified = evidence.status === 'within_range';
  const title = isAttention
    ? 'Review completion location'
    : isVerified
      ? 'Completion location verified'
      : 'Completion location not verified';
  const distance =
    evidence.varianceMeters === null
      ? evidence.reason || 'No trustworthy vehicle position was available at completion.'
      : `${evidence.varianceMeters.toLocaleString()} m from the planned stop • ${evidence.thresholdMeters.toLocaleString()} m review threshold`;

  return (
    <Alert
      severity={isAttention ? 'warning' : isVerified ? 'success' : 'info'}
      data-testid={`route-run-stop-location-evidence-${stopIndex}`}
    >
      <Typography variant="subtitle2">{title}</Typography>
      <Typography variant="body2">{distance}</Typography>
      <Typography variant="caption" color="text.secondary">
        Latest vehicle telemetry{evidence.telemetryAt ? ` at ${formatWhen(evidence.telemetryAt)}` : ''};
        not driver-device GPS.
      </Typography>
    </Alert>
  );
}
...
export default function RouteRunDetailPage() {
  ...
  const completionEvidenceByStop = useMemo(
    () =>
      Object.fromEntries(
        Object.entries(eventsByStop)
          .map(([stopId, events]) => [stopId, parseCompletionLocationEvidence(events)] as const)
          .filter(
            (entry): entry is readonly [string, CompletionLocationEvidence] =>
              entry[1] !== null,
          ),
      ),
    [eventsByStop],
                ...
                {(detail.stops || []).map((stop: RouteRunStopRecord, index: number) => (
                  <SurfacePanel key={stop.id} sx={{ bgcolor: 'rgba(30,26,23,1)' }} data-testid={`route-run-stop-card-${index}`}>
                    <Stack spacing={1.25}>
                      ...
                      {completionEvidenceByStop[stop.id] ? (
                        <CompletionLocationEvidencePanel
                          evidence={completionEvidenceByStop[stop.id]}
                          stopIndex={index}
                        />
                      ) : null}
