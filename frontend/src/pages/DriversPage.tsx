...
export default function DriversPage() {
  ...
  const handleSubmit = async () => {
    const { certifications, ...driverFields } = formData;
    const payload = {
      ...driverFields,
      certifications: parseCertifications(certifications),
    };
    try {
      if (editingDriver) {
        await updateDriverMutation.mutateAsync({ id: editingDriver.id, updates: payload });
      } else {
          ...
          {selectedDriver ? (
            <>
              ...
              <Box sx={{ borderTop: '1px solid #e3e8ef', mt: 2, pt: 1.5 }}>
                ...
                <Box sx={{ mt: 0.8 }}>
                  <Typography sx={{ fontSize: 12, color: 'text.secondary', mb: 0.65 }}>
                    Certifications used for route eligibility
                  </Typography>
                  <Stack direction="row" spacing={0.7} flexWrap="wrap" useFlexGap data-testid="driver-certifications">
                    {selectedDriver.certifications?.length ? selectedDriver.certifications.map((certification) => (
                      <Chip key={certification} size="small" color="info" label={certification} />
                    )) : (
                      <Chip size="small" variant="outlined" label="None recorded" />
                    )}
                  </Stack>
                </Box>
              </Box>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent sx={{ display: 'grid', gap: 2, pt: 2 }}>
          ...
          <TextField
            ...
            helperText="Comma-separated: hazmat, cold_chain, forklift, glass handling"
            onChange={(event) => setFormData((current) => ({ ...current, certifications: event.target.value }))}
          />
