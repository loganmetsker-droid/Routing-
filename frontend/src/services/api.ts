...
const AUTH_BYPASS = import.meta.env.VITE_AUTH_BYPASS === 'true' || import.meta.env.VITE_MOCK_PREVIEW === 'true';
...
const isLocalPreviewBypassActive = () => isLocalPreviewEnabled();
...
export const isAuthBypassed = () => isLocalPreviewBypassActive();
...
export const isAuthenticated = () => isAuthBypassed() || Boolean(getAuthToken());
...
export const createCustomer = async (customer: Partial<Customer>): Promise<{ customer: Customer }> => {
  const response = await apiFetch('/api/customers', {
    method: 'POST',
...
export const updateCustomer = async (id: string, updates: Partial<Customer>): Promise<{ customer: Customer }> => {
...
const isPreview = () => isLocalPreviewBypassActive();
