...
} from '../services/runtimeReadiness';
import { getTrovanDataMode } from '../services/dataMode';
...
const pageCopy: Record<string, [string, string]> = {
  ...
  '/academy': ['Trovan Academy', 'Role-based training, workspace tasks, and launch-readiness evidence'],
};
...
export function AppShell({ onLogout, children }: AppShellProps) {
  ...
  const runtimeStatus = workspaceRuntimeStatus(
    getTrovanDataMode(),
    runtimeReadinessQuery.data,
    runtimeReadinessQuery.isError,
  );
