...
test.describe('Step 3 migrated UI controls', () => {
  ...
  test('POD, exceptions, reports, and vehicles expose expected utility controls', async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Search fleet' })).toBeVisible();
    await expect(page.getByRole('combobox', { name: 'Operating state' })).toBeVisible();
    await expect(page.getByRole('combobox', { name: 'Sort' })).toBeVisible();
    await expect(page.getByText(/^All \d+$/, { exact: true })).toBeVisible();
  });
