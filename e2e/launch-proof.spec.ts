...
test.describe('launch calculation and control proof', () => {
  ...
  test('vehicle type totals reconcile with every saved vehicle record', async ({ page }) => {
    ...
    await expect(page.getByText(`All ${seed.vehicles.length}`, { exact: true })).toBeVisible();
    for (const [type, label] of Object.entries(profiles)) {
  ...
  test('dispatch and exception counts reconcile with route and job source records', async ({ page }) => {
    ...
    await expect(page.getByTestId('dispatch-execution-summary')).toContainText('Live execution');
    await expect(page.getByTestId('dispatch-execution-summary')).toContainText(
      'Missing telemetry is never inferred',
    );
    await expect(page.getByText('Routes by Execution Priority', { exact: true }).locator('..')).toContainText(String(seed.routes.length));
    const executionCards = page.locator('[data-testid^="dispatch-route-card-"]');
    await expect(executionCards).toHaveCount(seed.routes.length);
    await expect(executionCards.first()).toHaveAttribute('data-execution-health', 'delayed');
    expect(Number(await executionCards.first().getAttribute('data-execution-variance'))).toBeGreaterThanOrEqual(15);
    await expect(page.getByTestId('dispatch-route-card-route-gamma-003')).toContainText('Progress0 / 1');
    await expect(page.getByText('On track', { exact: true })).toHaveCount(0);

    await page.getByRole('button', { name: /Awaiting data/i }).click();
    await expect(page.locator('[data-testid^="dispatch-route-card-"]')).toHaveCount(2);
    await expect(page.locator('[data-testid^="dispatch-route-card-"]').first()).toHaveAttribute(
      'data-execution-health',
      'awaiting',
    );
    await page.getByRole('button', { name: /Delayed/i }).click();
    await expect(page.locator('[data-testid^="dispatch-route-card-"]')).toHaveCount(1);
    await expect(page.locator('[data-testid^="dispatch-route-card-"]').first()).toHaveAttribute(
      'data-execution-health',
      'delayed',
    );
  ...
  test('tracking and proof-of-delivery totals reconcile with route stops', async ({ page }) => {
    ...
    await expect(page.getByText('Vehicles reporting', { exact: true }).locator('..')).toContainText(String(liveRoutes.length));
    await expect(page.getByText('Newest recorded ping', { exact: true }).locator('..')).toContainText(/Just now|[1-5]\d?s ago|1m ago/);
    await expect(page.getByText('Fleet signal coverage', { exact: true }).locator('..')).toContainText(`${liveRoutes.length}/${liveRoutes.length}`);
    await expect(page.getByTestId('tracking-history-controls')).toContainText(/recorded history/i);
  ...
  test('route-run lifecycle, stop actions, messaging, proof, and exception controls persist their outcomes', async ({ context, page }) => {
    ...
    await expect(page.getByTestId('route-run-stop-location-evidence-0')).toContainText(
      'Completion location verified',
    );
    await expect(page.getByTestId('route-run-stop-location-evidence-0')).toContainText(
      '82 m from the planned stop',
    );
  ...
  test('settings state-dependent administration controls have honest outcomes', async ({ page }) => {
    ...
    await expect(page.getByLabel('SMS delivery')).toBeDisabled();
    await page.getByLabel('Send on-the-way email this many minutes before ETA').fill('45');
    await page.getByLabel('Completion location review threshold (meters)').fill('300');
    await page.getByLabel('Delivery failed or needs attention').uncheck();
    await page.getByRole('button', { name: 'Save settings', exact: true }).click();
    await expect(page.getByRole('alert').filter({ hasText: 'Organization settings saved.' })).toBeVisible();
    await expect(page.getByLabel('Send on-the-way email this many minutes before ETA')).toHaveValue('45');
    await expect(page.getByLabel('Completion location review threshold (meters)')).toHaveValue('300');
    conditionalControlProof.push({
      area: 'settings',
      controls: ['Notification timing', 'Previous-stop gate', 'Completion variance threshold'],
      status: 'passed',
    });
